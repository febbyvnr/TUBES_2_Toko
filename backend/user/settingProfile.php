...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($action === 'delete') {
    ...
    if ($password === '') {
    ...
    } elseif (!password_verify($password, $me['password'])) {
      ...
      if (!$del) {
        ...
        if ($del->execute()) {
          ...
          $deleteError = 'Gagal menghapus akun: ' . $del->error;
          $del->close();
        }
      }
    }
    ...
    if (empty($errors)) {
      ...
      if (!$up) {
        ...
        if ($up->execute()) {
          ...
          header('Location: /TUBES_2_Toko/backend/user/settingProfile.php');
          exit;
        } else {
          $errors[] = 'Gagal menyimpan: ' . $up->error;
          $up->close();
        }
      }
    }
...
if ($usernameSafe === '') {
  ...
  $initials = function_exists('mb_substr')
    ? strtoupper(mb_substr($usernameSafe, 0, 2))
    : strtoupper(substr($usernameSafe, 0, 2));
}

$avatarUrl = null;
$avatarFsPath = null;
if (!empty($me['profile_photo'])) {
  $avatarUrl = '/TUBES_2_Toko/assets/profile/' . rawurlencode($me['profile_photo']);
  $avatarFsPath = __DIR__ . '/../assets/profile/' . $me['profile_photo'];
}
?>
<!doctype html>
<html lang="en">
<head>
  ...
  <link rel="stylesheet" href="/TUBES_2_Toko/frontend/styles/SettingProfile.css">
  <link rel="stylesheet"
...
<main class="container" style="padding:28px 20px; max-width:800px;">
  ...
  <div style="display:flex; justify-content:center; margin-bottom:20px;">
    <div class="avatar-wrapper"
         style="
          ...
          background:#f5f5f5;font-weight:600;font-size:32px;color:#555;">
  ...
  <?php if (!empty($_SESSION['flash_success'])): ?>
    <div style="max-width:640px;margin:0 auto 12px;padding:10px 16px;border-radius:8px;background:#e7f9ee;border:1px solid #9bd4af;color:#236b3d;font-size:14px;">
  ...
  <?php if (!empty($errors)): ?>
    <div style="max-width:640px;margin:0 auto 12px;padding:10px 16px;border-radius:8px;background:#ffe9e9;border:1px solid #ffb3b3;color:#b00000;font-size:14px;">
